...
async function registerDriver({ email, password }: { email: string; password: string }) {
  try {
    const res = await fetch("https://trucklink.onrender.com/api/driver/registerDriver", {
      method: "POST",
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  const success = await registerDriver({ email: form.email, password: form.password });
    ...
    <main className="min-h-screen bg-gradient-to-br from-white via-blue-300 to-white p-2 flex flex-col items-center font-sans text-black animate-fade-in">
      ...
      <div className="w-full max-w-md bg-white/25 rounded-3xl shadow-2xl border border-white/30 backdrop-blur-md p-8 space-y-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-black/80 text-sm">Sign up as a Driver</p>
        </div>
        ...
        <div className="text-center space-y-3 text-sm text-black/80">
          <p>
            ...
            <Link to="/driver/login" className="underline font-semibold text-indigo-900 hover:text-indigo-700">
              Sign in
            </Link>
          </p>
          <p>
            <Link to="/manager/signup" className="underline text-indigo-900/70 text-xs hover:text-indigo-700">
              Want to sign up as a manager?
            </Link>
          </p>
        </div>
      </div>
